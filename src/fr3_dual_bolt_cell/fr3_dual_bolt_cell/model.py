...
def add_gripper(root, side, cfg):
    ...
    # The real HKV driver exposes one scalar opening command.  Reuse the
    # first finger joint as that hardware joint and make the second finger
    # mimic it, avoiding a second exported hardware DOF.
    # Every backend uses the same scalar opening coordinate. The second finger
    # is a visual/collision mimic; ros2_control always exports one joint.
    for index, sign in enumerate((-1, 1)):
        finger = 'left' if index == 0 else 'right'
        link_name = p+finger+'_finger'
        joint_name = p+'gripper_joint' if index == 0 else p+finger+'_finger_joint'
        ...
        element(joint, 'limit', lower=0, upper=.1 if index == 0 else .1,
                effort=100, velocity=0.10)
        element(joint, 'dynamics', damping=2.0, friction=.10)
        if index == 1:
            # Both model joints use the same positive opening coordinate;
            # their opposite origins/axes provide the mirrored geometry.
            element(joint, 'mimic', joint=p+'gripper_joint', multiplier=1.0)
...
def controllers(mode, side=None):
    ...
    for arm in SIDES if side is None else (side,):
        ...
        result[names[2]] = {'ros__parameters': {
            'joint': arm+'_gripper_joint', 'goal_tolerance': .002,
