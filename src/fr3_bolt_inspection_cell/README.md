...
默认启用 `handover_assist: true`，让递交手配合寻找双方都能接近的姿态。优先绕螺丝长轴旋转 +90°，其次 -90°，再尝试原姿态；这会改变夹爪相对位置，保持螺丝中心和长轴方向不变，不是固定转动第六关节。仍不可达时，尝试配置中的 60°/120° 和小范围位移：向前 3 cm、向接收手一侧 3 cm，最后一个候选再下移 3 cm。两只夹爪始终保持互相垂直，初始对称姿态不变。

每个候选先规划右手路径，再以该路径终点的完整双臂状态预检左手最终抓取、直线接近和连接路径。此阶段不执行机器人运动，也不把候选关节角写入 Gazebo。递交手移动期间不豁免零件与接收手的碰撞。选出可行组合后才移动右手；到位并验证持物后，重新读取零件和双臂实测状态，再完整规划左手接近。搜索失败不发送运动；执行或夹持核验失败不会自动尝试下一个候选。

候选顺序可在 `handover_donor_candidates` 调整：`roll_deg` 是相对展示基准姿态绕零件局部 X 长轴的角度，`offset` 为相对交接中心的世界 XYZ 偏移，单位米。左手接收时正 Y 指向左侧，右手接收时自动镜像 Y；最多 8 个候选，角度不超过 ±180°，位移长度不超过 6 cm。搜索选取第一个通过完整预检的组合，不保证全局最优姿态。日志 `Cooperative handover` 显示候选与拒绝原因；关节逆解及连续路径检查可能需要较长时间。将 `handover_assist: false` 可恢复固定交接位姿流程。

垂直交接优先预检原抓取点的两个侧向接近方向及两种腕部翻转。若四种姿态均失败，再尝试头部中心的四种姿态，让左右手沿零件轴线错开。备用偏移按 `(bolt_length-head_length)/2` 计算，当前为 18.5 mm；原偏移为 10 mm。所有候选始终保持夹爪相互垂直。每个候选先求最终夹取点的碰撞感知逆解，再反向验证直线接近段，最后规划从当前状态到预接近位置的连接；三部分全部通过才发送左手运动。执行失败不会自动换姿态继续移动。所有候选失败时保留夹持，日志输出逆解错误与诊断碰撞对象；关闭碰撞的逆解仅用于诊断，不用于执行。
...
交接采用链环式垂直姿态：接收夹爪绕零件轴线相对递交夹爪旋转 90°，从侧面接近；两手闭合方向互相垂直。优先使用原轴向抓取偏移，碰撞或不可达时再向头部中心错开。闭合前核验实际 TCP 位置及姿态。先确认双指持续接触并切换持有者，递交手张开、撤离后，接收手移动 10 mm 验证零件跟随，再继续拍摄。

界面新增“跳到左手交接”（服务 `/inspection/skip_to_handover`）。右手进入展示阶段后可点击：当前运动完成后跳过剩余右手视角，进入左手交接，不会同时发出另一组运动。右手展示失败或停止后，只要本次任务保留交接上下文，也可点击恢复。恢复会等待机械臂静止、确认右手仍持有零件、更新碰撞场景，再规划到交接姿态。左手接管验证通过后右手才松开。如果左手候选全部预检失败或预检被停止，左手尚未开始执行，后台退出后仍可点击此按钮重试。只有在完整预检通过、即将发送左手运动时才清除恢复上下文；执行或交接中途失败不允许直接重入。重启节点后不保留恢复上下文。手动跳过会记录在报告中，左手仍执行正常拍摄流程。

`config/inspection.yaml` 中展示请求速度为 `scan_speed: 0.030` m/s、`scan_joint_speed: 0.24` rad/s、`scan_joint_acceleration: 0.60` rad/s²。仅定点展示旋转使用这些参数，下降和交接沿用各自速度。launch 将实际生成的 MoveIt 关节限速写入临时任务配置；手动生成的笛卡尔轨迹在应用倍率后仍受这些上限约束。当前机械臂模型上限为 0.30 rad/s、0.30 rad/s²，所以 2× 不保证耗时减半，0.60 rad/s² 的请求也会限制到模型允许值。日志显示限幅前后数值和展示轨迹的仿真时长；实际耗时还受逐点停靠和 Gazebo 实时率影响。

任务状态连接过期时，滑条禁用并取消尚未发送的速度调整；恢复连接后显示后端实际倍率。已开始执行的轨迹不会因 UI 断连而改变速度。

笛卡尔轨迹首点的接入时间也考虑最新实测关节位置：允许范围内的偏差按关节限速平缓纠正，不再固定用 0.1 秒；偏差超过 0.01 rad 时要求重新规划。
...
面板还可切换四路相机图像、显示当前阶段，并提供 **停止运动并保持夹持**。相机超过 2 秒未收到新帧会清空旧画面并显示过期；切换到没有图像或格式不支持的相机时显示对应原因，新帧恢复后自动刷新。启动 launch 不会自动运动。停止是 ROS 动作取消，不是实机急停；本版没有实机连接。
...
| `inspection_center`、`handover_center` | 展示 `[0.25, 0, 1.15]`、交接 `[0.35, 0, 1.00]`，世界坐标，单位米 |
| `views_deg` | 绕零件局部 X/Y/Z 的角度，单位度；每次拍摄后回基准姿态 |
| `center_tolerance` | 0.003 m，规划路径中心误差阈值；仿真运动中也检查中心漂移 |
| `grasp_reach_tolerance` | 0.001 m，闭合前实际 TCP 的三维到位容差 |
