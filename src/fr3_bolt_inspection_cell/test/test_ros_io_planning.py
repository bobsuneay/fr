...
def adapter(monkeypatch):
    ...
    io.c['joint_limits'] = {f'{side}_j{i}': dict(has_velocity_limits=True,
        max_velocity=.3, has_acceleration_limits=True, max_acceleration=.3)
...
def test_scan_speed_changes_only_centered_rotation(adapter):
    ...
    assert seconds(scan) < seconds(normal)
    assert 5.8*.1/seconds(scan)**2 <= .3+1e-8


def test_live_slider_retimes_only_next_scan_not_existing_or_descent(adapter):
    _, io = adapter
    io.c.update(scan_joint_speed=.06, scan_joint_acceleration=.05)
    q = np.array([[0.0], [.1]])
    frames = [np.eye(4)]*2
    before, after, descent = trajectory(), trajectory(), trajectory()
    descent_before = trajectory()
    io.run_cartesian_trajectory(descent_before, q, frames, .03)
    ...
    assert span(descent) == span(descent_before)


@pytest.mark.parametrize('scanning', [False, True])
def test_retimed_motion_cannot_exceed_model_limits(adapter, scanning):
    _, io = adapter
    io.c.update(joint_speed=2., joint_acceleration=4.,
                scan_joint_speed=.24, scan_joint_acceleration=.6)
    io.n.scan_speed_scale = 2.0
    io.c['joint_limits']['right_j1']['max_velocity'] = .1
    io.c['joint_limits']['right_j1']['max_acceleration'] = .07
    planned = trajectory()
    frames = [np.eye(4)]*2
    io.run_cartesian_trajectory(planned, np.array([[0.], [.1]]), frames, .03,
        np.eye(4) if scanning else None, np.zeros(3) if scanning else None)
    a, b = [p.time_from_start for p in planned.joint_trajectory.points]
    dt = b.sec-a.sec+(b.nanosec-a.nanosec)*1e-9
    assert 1.875*.1/dt <= .1+1e-8
    assert 5.8*.1/dt**2 <= .07+1e-8
    io.execute.assert_called_once_with(planned)


def test_missing_model_limit_never_executes_trajectory(adapter):
    _, io = adapter
    del io.c['joint_limits']['right_j1']
    with pytest.raises(ValueError, match='Missing MoveIt motion limits for right_j1'):
        io.run_cartesian_trajectory(trajectory(), np.array([[0.], [.1]]), [np.eye(4)]*2, .03)
    io.execute.assert_not_called()


@pytest.mark.parametrize('start_delta', [.005, .02])
def test_measured_start_is_timed_or_rejected_before_cartesian_execution(adapter, start_delta):
    module, io = adapter
    io.state = lambda: NS(joint_state=NS(name=['right_j1'], position=[start_delta]))
    planned = trajectory()
    if start_delta > .01:
        with pytest.raises(module.PlanningFailure, match='Cartesian start changed'):
            io.run_cartesian_trajectory(planned, np.array([[0.], [.1]]), [np.eye(4)]*2, .03)
        io.execute.assert_not_called()
        return
    io.run_cartesian_trajectory(planned, np.array([[0.], [.1]]), [np.eye(4)]*2, .03)
    first = planned.joint_trajectory.points[0].time_from_start
    dt = first.sec+first.nanosec*1e-9
    assert dt > .1
    assert 1.875*start_delta/dt <= io.c['joint_speed']+1e-8
    assert 5.8*start_delta/dt**2 <= io.c['joint_acceleration']+1e-8
    io.execute.assert_called_once_with(planned)
...
def test_virtual_pose_planning_uses_donor_state_without_reading_or_mutating_actual(adapter):
    _, io = adapter
    virtual = NS(joint_state=NS(name=['right_j1', 'left_j1'], position=[.8, .1]), is_diff=True)
    io.state = MagicMock(side_effect=AssertionError('Must retain hypothetical donor state'))
    planned = trajectory()
    io.action = MagicMock(return_value=NS(error_code=NS(val=1), planned_trajectory=planned))
    assert io.global_move('left', target=np.eye(4), plan_only=True, start_state=virtual) is planned
    request = io.action.call_args.args[1].request
    assert request.start_state is not virtual
    assert request.start_state.joint_state.position == [.8, .1]
    request.start_state.joint_state.position[0] = 0.
    assert virtual.joint_state.position == [.8, .1]
    io.execute.assert_not_called()
    with pytest.raises(ValueError, match='planning only'):
        io.global_move('left', target=np.eye(4), start_state=virtual)


def test_virtual_pick_keeps_donor_state_for_ik_and_receiver_connection(adapter):
    _, io = adapter
    names = [f'{side}_j{i}' for side in ('left', 'right') for i in range(1, 7)]
    virtual = NS(joint_state=NS(name=names, position=[.5]*6+[.8]*6), is_diff=True)
    io.state = MagicMock(side_effect=AssertionError('Unexpected actual state read'))
    io.ik = object()
    def ik(client, request):
        assert request.ik_request.robot_state.joint_state.position[6:] == [.8]*6
        return NS(error_code=NS(val=1), solution=NS(joint_state=NS(name=names, position=[.3]*6+[.8]*6)))
    io.call = ik
    connection = NS(joint_trajectory=NS(joint_names=names[:6], points=[NS(positions=[.4]*6)]))
    io.validate_robot_state = MagicMock()
    io.seeded_cartesian = MagicMock(return_value=(connection, np.array([[.3]*6, [.4]*6]), [np.eye(4)]*2))
    io.global_move = MagicMock(return_value=connection)
    _, prepared = io.pick_approach('left', np.eye(4), np.eye(4), plan_only=True, start_state=virtual)
    assert io.global_move.call_args.kwargs['start_state'].joint_state.position == [.5]*6+[.8]*6
    assert prepared.start.joint_state.position == [.4]*6+[.8]*6
    assert virtual.joint_state.position == [.5]*6+[.8]*6
    io.execute.assert_not_called()
    with pytest.raises(ValueError, match='planning only'):
        io.pick_approach('left', np.eye(4), np.eye(4), start_state=virtual)
