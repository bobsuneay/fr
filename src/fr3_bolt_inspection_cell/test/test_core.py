...
from fr3_bolt_inspection_cell.core import (SimClockDeadline, centered_views, estimate_bolt,
    ...
    transform, validate, trajectory_joint_limits)
...
def test_timing_limits_follow_generated_moveit_model_and_active_joints():
    from fr3_dual_bolt_cell.model import moveit_config
    root, arms = robot()
    limits = moveit_config(root, arms, BASE)['robot_description_planning']['joint_limits']
    names = ['left_j1', 'left_j2', 'left_j3']
    assert trajectory_joint_limits(names, limits) == pytest.approx([.3, .3])
    limits['left_j2']['max_velocity'] = .18
    limits['left_j3']['max_acceleration'] = .09
    # An inactive arm's slower limit must not slow this trajectory.
    limits['right_j1']['max_velocity'] = .01
    assert trajectory_joint_limits(names, limits) == pytest.approx([.18, .09])


@pytest.mark.parametrize('invalid', [0., -1., float('nan'), float('inf')])
@pytest.mark.parametrize('field', ['max_velocity', 'max_acceleration'])
def test_invalid_model_timing_limits_rejected(invalid, field):
    entry = dict(has_velocity_limits=True, max_velocity=.3,
                 has_acceleration_limits=True, max_acceleration=.3)
    entry[field] = invalid
    with pytest.raises(ValueError, match='Invalid MoveIt motion limits for left_j1'):
        trajectory_joint_limits(['left_j1'], {'left_j1': entry})


@pytest.mark.parametrize('field', ['speed', 'joint_speed', 'acceleration', 'xyz'])
def test_nonfinite_retiming_inputs_rejected(field):
    kwargs = dict(speed=.03, joint_speed=.12, acceleration=.15)
    xyz = np.zeros((2, 3))
    if field == 'xyz':
        xyz[-1, -1] = np.nan
    else:
...
@pytest.mark.parametrize('candidate', [dict(roll_deg=181, offset=[0, 0, 0]),
    dict(roll_deg=90, offset=[.07, 0, 0]), dict(roll_deg=90, offset=[0, 0]),
    dict(roll_deg=float('nan'), offset=[0, 0, 0])])
def test_cooperative_handover_candidates_are_bounded(candidate):
    c = config()
    c['handover_donor_candidates'] = [candidate]
    with pytest.raises(ValueError):
        validate(c)
...
def test_axially_staggered_original_fingers_clear_logged_overlap():
    from scipy.spatial import ConvexHull
    from scipy.optimize import linprog
    from fr3_bolt_inspection_cell.geometry import fingertip_geometry
    from fr3_bolt_inspection_cell.core import perpendicular_receiver_grasp
    root, _ = robot()
    def hulls(side, grasp, positions):
        geometry = fingertip_geometry(root, side,
            lambda uri: BASE / uri.removeprefix('package://fr3_dual_bolt_cell/'))
        return [ConvexHull((np.asarray(g['vertices_tcp'])+np.asarray(g['axis_tcp'])*q)
                           @grasp[:3, :3].T+grasp[:3, 3]).equations
                for g, q in zip(geometry, positions)]
    donor = grasp_in_object(-.010)
    donor[2, 3] = -.0068
    right = hulls('right', donor, [.0067, .0073])
    def overlap(offset, gap):
        receiver = perpendicular_receiver_grasp(offset, donor)
        receiver[:3, :3] = transform(rpy=(np.pi, 0, 0))[:3, :3]@receiver[:3, :3]
        for a in right:
            for b in hulls('left', receiver, [gap/2]*2):
                planes = np.vstack([a, b])
                if linprog([0]*3, A_ub=planes[:, :3], b_ub=-planes[:, 3],
                           bounds=[(None, None)]*3, method='highs').success:
                    return True
        return False
    assert overlap(.010, .035)
    for gap in (.035, .027, .0192):
        assert not overlap(.0185, gap)
