...
def validate(cfg):
    ...
    finite(cfg)
    cfg.setdefault('handover_assist', False)
    if type(cfg['handover_assist']) is not bool:
        raise ValueError('handover_assist must be true or false')
    if cfg['handover_assist']:
        candidates = cfg.get('handover_donor_candidates', [])
        if not isinstance(candidates, list) or not 1 <= len(candidates) <= 8:
            raise ValueError('Require 1 to 8 handover donor candidates')
        for candidate in candidates:
            if (not isinstance(candidate, dict) or 'roll_deg' not in candidate
                    or 'offset' not in candidate or len(candidate['offset']) != 3
                    or abs(candidate['roll_deg']) > 180
                    or np.linalg.norm(candidate['offset']) > .06):
                raise ValueError('Handover donor candidates require roll within 180 deg and XYZ offset within 6 cm')
...
def trajectory_joint_limits(names, limits):
    """Conservative scalar bounds from the active trajectory's MoveIt joints."""
    if not names:
        raise ValueError('Trajectory has no joint names')
    bounds = []
    for name in names:
        entry = limits.get(name, {})
        if not (entry.get('has_velocity_limits') is True and
                entry.get('has_acceleration_limits') is True):
            raise ValueError(f'Missing MoveIt motion limits for {name}; restart with bringup.launch.py')
        values = [entry.get('max_velocity', 0), entry.get('max_acceleration', 0)]
        if any(not math.isfinite(v) or v <= 0 for v in values):
            raise ValueError(f'Invalid MoveIt motion limits for {name}')
...
def segment_times(positions, tcp_positions, speed, joint_speed, acceleration,
    ...
    if (q.ndim != 2 or q.shape[1] == 0 or len(q) < 2 or xyz.shape != (len(q), 3)
            or not np.all(np.isfinite(q)) or not np.all(np.isfinite(xyz))):
        raise ValueError('Invalid Cartesian trajectory')
    if any(not math.isfinite(v) or v <= 0 for v in (speed, joint_speed, acceleration)):
    ...
    if start_positions is not None:
        ...
        if start.shape != q[0].shape or not np.all(np.isfinite(start)):
            raise ValueError('Invalid measured trajectory start')
        first_delta = np.max(np.abs(q[0]-start))
        first_time = max(first_time, 1.875*first_delta/joint_speed,
                         math.sqrt(5.8*first_delta/acceleration))
    ...
    return np.r_[first_time, first_time+np.cumsum(dt)]
