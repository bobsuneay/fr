...
def main():
    ...
    def receive(name, msg):
        with lock:
            frames[name] = (msg, time.monotonic())
    ...
    def update():
        ...
        camera = ui.selected.get()
        with lock:
            sample = frames.get(camera)
        ...
        changed, msg, placeholder = preview.select(camera, sample, time.monotonic())
        if changed and msg is None:
            ui.preview.configure(image='', text=placeholder)
            ui.preview.image = None
        elif changed:
            try:
