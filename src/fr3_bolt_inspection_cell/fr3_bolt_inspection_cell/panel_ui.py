...
class InspectionPanel:
    def __init__(self, root, cameras, request, set_speed=None):
        ...
        self.root, self.set_speed = root, set_speed
        self.speed_slider = ttk.Scale(speed_bar, from_=.25, to=2.0,
            variable=self.speed_value, command=self.speed_changed, length=250, state='disabled')
    ...
    def cancel_speed_change(self):
        if self.speed_timer is not None:
            self.root.after_cancel(self.speed_timer)
            self.speed_timer = None

    def speed_changed(self, value):
        if not self.speed_available:
            return
        self.speed_label.set(f'设定 {float(value):.2f}× · 等待确认（下一段生效）')
        self.cancel_speed_change()
        self.speed_timer = self.root.after(250, self.send_speed_change)

    def send_speed_change(self):
        self.speed_timer = None
        if self.speed_available and self.set_speed is not None:
            self.set_speed(round(self.speed_value.get(), 2))

    def controls(self, state, pending=False):
        confirmed = state.get('scan_speed_scale')
        self.speed_available = confirmed is not None
