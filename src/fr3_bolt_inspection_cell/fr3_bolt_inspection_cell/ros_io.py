...
from .core import SimClockDeadline, segment_times, trajectory_joint_limits
...
class IO:
    ...
    def tcp_pose(self, side, robot_state=None):
        """FK from fresh measurements, or an explicit planning-only robot state."""
        req = GetPositionFK.Request()
        req.header.frame_id = 'world'
        req.fk_link_names = [side+'_gripper_tcp']
        req.robot_state = self.state() if robot_state is None else deepcopy(robot_state)
    ...
    def global_move(self, side, target=None, joints=None, continuation=None, plan_only=False,
        ...
        if start_state is not None and not plan_only:
            raise ValueError('Hypothetical start state is for planning only')
        if continuation is not None:
            return self.pick_approach(side, target, continuation, plan_only=plan_only,
                                      start_state=start_state)
        goal = MoveGroup.Goal()
        req = goal.request
        req.group_name = side+'_arm'
        req.start_state = self.state() if start_state is None else deepcopy(start_state)
    ...
    def pick_approach(self, side, above, grasp, plan_only=False, start_state=None):
        ...
        if start_state is not None and not plan_only:
            raise ValueError('Hypothetical start state is for planning only')
        start = self.state() if start_state is None else deepcopy(start_state)
        ...
        for attempt, signs in enumerate(itertools.product((1, -1), repeat=3), 1):
            ...
            try:
                ...
                connection = self.global_move(side, joints=q[0], plan_only=True,
                    **({'start_state': start} if start_state is not None else {}))
    ...
    def run_cartesian_trajectory(self, trajectory, q, frames, speed, object_tcp=None, center=None):
        ...
        max_speed, max_acceleration = trajectory_joint_limits(
            trajectory.joint_trajectory.joint_names, self.c.get('joint_limits', {}))
        requested_speed, requested_acceleration = joint_speed, acceleration
        joint_speed = min(joint_speed, max_speed)
        acceleration = min(acceleration, max_acceleration)
        if joint_speed < requested_speed or acceleration < requested_acceleration:
            self.n.get_logger().info(
                f'Cartesian timing limited by MoveIt model: joint speed '
                f'{requested_speed:.3f} -> {joint_speed:.3f} rad/s, acceleration '
                f'{requested_acceleration:.3f} -> {acceleration:.3f} rad/s^2')
        measured = self.state().joint_state
        values = dict(zip(measured.name, measured.position))
        start = np.array([values[name] for name in trajectory.joint_trajectory.joint_names])
        if np.max(np.abs(start-q[0])) > .01:
            raise PlanningFailure('Cartesian start changed by more than 0.01 rad; replan before execution')
        # The controller interpolates from its current state to point zero too.
        # Account for permitted start error instead of always correcting in 0.1 s.
        times = segment_times(q, [t[:3, 3] for t in frames], speed, joint_speed,
                              acceleration, start_positions=start)
        if times[0] > .1:
            self.n.get_logger().info(
                f'Cartesian start correction: max_delta={np.max(np.abs(start-q[0])):.5f} rad, '
                f'first_point_time={times[0]:.3f} s')
        if scanning:
            self.n.get_logger().info(
