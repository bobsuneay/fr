...
from .handover import transfer, approach_receiver, prepare_cooperative_handover
...
class Inspection(Node):
    ...
    def _begin_receiver_motion(self):
        # A failed or cancelled preflight leaves the receiver unmoved and may be
        # retried. Invalidate the shortcut only when execution is about to start.
        self.io.check()
        self.handover_context = None
    ...
    def finish_handover(self, first, second, neutral, donor_grasp, receiver_grasp):
        ...
        if self.cfg.get('handover_assist', False):
            self.publish('HANDOVER_SEARCH', 'Preflight donor adjustment and receiver approach together')
            handover = prepare_cooperative_handover(self.io, first, second, handover,
                donor_grasp, receiver_grasp, self.cfg, self.settle, self.verify,
                self.get_logger().info)
            # Replan the receiver from measured feedback after the donor arrives.
            handover = self.io.truth()@np.linalg.inv(self.object_truth)
            self.io.object_scene(handover, first)
        else:
            self.io.global_move(first, handover@donor_grasp)
            self.settle()
            self.verify(handover)
        self.report['handover_object_pose'] = handover.tolist()
        ...
        receiver_grasp, target = approach_receiver(self.io, second, handover, receiver_grasp,
            ...
            alternative_offsets=[(self.cfg['bolt_length']-self.cfg['head_length'])/2],
            before_execute=self._begin_receiver_motion)
        ...
        self.get_logger().info(
            ...
            f'angle_error={angle_error:.5f} rad, axial_offset={receiver_grasp[0, 3]*1000:.2f} mm')
        if position_error > .001 or angle_error > .02:
            raise RuntimeError('Receiver TCP has not reached selected grasp; donor retains object')
